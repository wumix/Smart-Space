...
#		$this->CONF['energySpeed']	= 1;
	...
	private function ExecCalc()
	{
		...
		{
		$this->USER['darkmatter'] = $this->USER['darkmatter'] + ($this->ProductionTime * $this->PLANET['darkmatter_perhour'] / 3600);	
		}
...
{	
			...
			foreach($ressIDs as $ID) 
			{
				...
				if($Production > 0) {					
					...
					if(in_array($ID, $reslist['resstype'][4]) && ( $ID != 921 ? $this->PLANET[$resource[$ID]] == 0 : $this->USER[$resource[$ID]] == 0)) {
						 continue;
					}
		...
		if($this->PLANET['energy_used'] == 0) {
			...
			$this->PLANET['darkmatter_perhour']	= $temp[921]['plus'];
		} else {
