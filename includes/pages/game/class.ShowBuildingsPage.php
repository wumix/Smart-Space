...
class ShowBuildingsPage extends AbstractPage
{
	...
	private function getQueueData()
	{
		...
		foreach($buildQueue as $BuildArray) {
			...
			$scriptData[] = array(
				...
				'percent' => (int)min(($BuildArray[3]-TIMESTAMP / $BuildArray[3] * 100),100),
	...
	public function show()
	{
		...
		foreach($Elements as $ID => $Element)
		{
			...
			$destroyTime		= BuildFunctions::getBuildingTime($USER, $PLANET, $Element, 0);
		...
		$this->tplObj->assign_vars(array(
			...
			'RoomIsOk'			=> $RoomIsOk,
			'Queue'				=> $Queue,
			'mod'			=> array('resourcegate' => $PLANET['resourcegate'] == 1),
			...
			'proc_field' => (int)min(($PLANET['field_current'] / CalculateMaxPlanetFields($PLANET) * 100),100),
			));
