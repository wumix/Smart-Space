...
class ShowSettingsPage extends AbstractPage
{
	...
	public function show()
	{
		...
		else
		{
			$this->tplObj->assign_vars(array(				
				...
				'mobile'	=> $USER['mobile_version'],	
				'navbar'	=> $USER['fixed_navbar'],	
				'header'	=> $USER['fixed_header'],	
	...
	private function sendDefault()
	{
		...
		$mobile	= HTTP::_GP('mobile', 0);
		$navbar	= HTTP::_GP('navbar', 0);
		$header	= HTTP::_GP('header', 0);
		...
		if($mobile == 1) {
			$SQL	.= "UPDATE ".USERS." SET mobile_version = 'initial-scale=0.0' WHERE id = ".$USER['id'].";";
		} else {
			$SQL	.= "UPDATE ".USERS." SET mobile_version = 'initial-scale=1.0' WHERE id = ".$USER['id'].";";
		}
	
		if($navbar == 1) {
			$SQL	.= "UPDATE ".USERS." SET fixed_navbar = 'fixed-navigation' WHERE id = ".$USER['id'].";";
		} else {
			$SQL	.= "UPDATE ".USERS." SET fixed_navbar = '' WHERE id = ".$USER['id'].";";
		}
	
		if($header == 1) {
			$SQL	.= "UPDATE ".USERS." SET fixed_header = 'fixed-header' WHERE id = ".$USER['id'].";";
		} else {
			$SQL	.= "UPDATE ".USERS." SET fixed_header = '' WHERE id = ".$USER['id'].";";
		}
